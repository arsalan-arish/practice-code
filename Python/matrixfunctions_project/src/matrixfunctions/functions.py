...
def minor3x3(
    A, n
):  # n is a List representing the index of the element which's minor is to be found
...
def cofactor3x3(
    A, n
):  # n is a List representing the index of the element which's cofactor has to be found
...
def scalarmultiply(
    A, n
):  # This function can multiply matrix of any order with a scalar
    for i in range(len(A)):
        for j in range(len(A[i])):
            A[i][j] *= n
...
def determinant(A):
    if len(A) == 2 and len(A[0]) == 2:
        ...
        ans = (
            A[0][0] * minor3x3(A, [0, 0])
            - A[0][1] * minor3x3(A, [0, 1])
            + A[0][2] * minor3x3(A, [0, 2])
        )
